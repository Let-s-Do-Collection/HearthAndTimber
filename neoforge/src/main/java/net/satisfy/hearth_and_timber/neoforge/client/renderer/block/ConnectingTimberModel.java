...
public class ConnectingTimberModel implements BakedModel {
    ...
    public static int getCardinals(ModelData data, Direction face) {
        ...
        assert face != null;
        Pair<Direction, Direction> axis = new Pair<>(TimberHelper.getPer1(face), TimberHelper.getPer2(face));
        Function<Vec3i, Boolean> exists = (vec) -> {
            assert data != null;
            return TimberHelper.exists(vec, Objects.requireNonNullElse(data.get(CONNECTIONS), 0));
        };
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, RandomSource rand, ModelData data, @Nullable RenderType renderType) {
        if (data == null || !data.has(CONNECTIONS)) {
            assert rand != null;
            return original.getQuads(state, side, rand);
        }
        assert rand != null;
